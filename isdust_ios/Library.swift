...
class Library{
    ...
    func login(user:String,password:String) -> String {
        ...
        for i in res{
            temp_array.append((text_web! as NSString).substring(with: i.range(at: 1)))
    ...
    func get_borrwingdetail() -> [[String]] {
        ...
        for i in res{
            ...
            for j in 0..<4{
                temp[j]=(text_web! as NSString).substring(with: i.range(at: j+1))
    ...
    func getState(text:String) -> String {
        ...
        let data = source.data(using: String.Encoding.utf8) //data  是json格式字符串
        let json = try? JSONSerialization.jsonObject(with: data!)
        return (json![text]!!["stateName"] as? String)!
        
    }
    func getReturnDate(source:String,barcode:String) -> String {
        let data = source.data(using: String.Encoding.utf8) //data  是json格式字符串
        let json = try? JSONSerialization.jsonObject(with: data!)
        return (json![barcode]!!["returnDate"] as? String)!
    }
    ...
    func AnalyzeStorage(text:String) -> [[String]] {
        var result:[[String]] = [[String]] ()
        var raw_bookinfo="[{\""+mhttp.getMiddleText(text,"[{\"","}]")+"}]";
        ...
        for i in 0 ..< json!.count{
            ...
            temp[0]=json![i]["callno"]as! String
            temp[1]=json![i]["barcode"] as! String
            temp[2]=getState(text:String( json![i]["state"] as! Int))
            if(temp[2]=="借出"){
            temp[3]=TimeStamp2Date(timeStamp: getReturnDate(source: raw_borrowinfo!,barcode: temp[1] ));
            }else{
            temp[3]=""
            }
            temp[4]=json![i]["orglib"] as! String
            temp[5]=json![i]["orglocal"] as! String
    ...
    func AnalyzeSearch(text:String) -> [Book] {
        ...
        let regex_all = try! RegularExpression(pattern: expression_all, options: RegularExpression.Options.caseInsensitive)
        let regex_name = try! RegularExpression(pattern: expression_name, options: RegularExpression.Options.caseInsensitive)
        var res_all = regex_all.matches(in: text, options: RegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, text.characters.count))
        
        
        var url_suoshuhao="http://interlib.sdust.edu.cn/opac/book/callnos?bookrecnos="
        
        for i in res_all{
            var text_all=(text as NSString).substring(with: i.range(at: 1))
            var res_name = regex_name.matches(in: text_all, options: RegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, text_all.characters.count))
            var book_name=(text_all as NSString).substring(with: res_name[0].range(at: 1))
            ...
            temp_book.writer=mhttp.getMiddleText(text_all, "?searchWay=author&q=", "\" target=\"_blank\"> ")
            temp_book.publisher=mhttp.getMiddleText(text_all,  "?searchWay=publisher&q=", "\" target=\"_blank\"> ")
            temp_book.publishedday=mhttp.getMiddleText(text_all, "出版日期: ","</div>").replacingOccurrences(of: "\n", with: "").replacingOccurrences(of: "\r", with: "").replacingOccurrences(of: "\t",with: "")
            temp_book.bookrecno=mhttp.getMiddleText(text_all,  "express_bookrecno=\"", "\" express_isbn=")
            temp_book.ISBN=mhttp.getMiddleText(text_all, "express_isbn=\"", "\" express_bookmeta_").replacingOccurrences(of: "-", with: "")
